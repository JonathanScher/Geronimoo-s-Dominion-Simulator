...
public class DomPlayer implements Comparable<DomPlayer> {
	...
	private EnumMap<DomCardName, DomPlayStrategy> playStrategies = new EnumMap<DomCardName, DomPlayStrategy>(
			DomCardName.class);
	...
	public void makeBuyDecision() {
		for (DomBuyRule theBuyRule : getBuyRules()) {
			if (theBuyRule.getCardToBuy().hasCardType(DomCardType.Prize))
				continue;
			if (getTotalAvailableCurrency().compareTo(
					theBuyRule.getCardToBuy().getCost(getCurrentGame())) < 0)
				continue;
	...
	private void updateVPCurve(boolean isCumulative) {
		if (VPcurve.size() < getTurns()) {
			...
			if (isCumulative)
				VPcurve.set(getTurns() - 1, VPcurve.get(getTurns() - 1)
						+ countVictoryPoints());
			else
				VPcurve.set(getTurns() - 1, VPcurve.get(getTurns() - 1)
						+ countVictoryPoints() - pointsBeforeBuys);
		}
	}
	...
	private void updateMoneyCurve() {
		if (moneyCurve.size() < getTurns()) {
			...
			moneyCurve.set(getTurns() - 1, moneyCurve.get(getTurns() - 1)
					+ availableCoins);
		}
	}
	...
	protected boolean tryToBuy(DomCardName aCardName) {
		...
		if (aCardName == DomCardName.Grand_Market
				&& !getCardsFromPlay(DomCardName.Copper).isEmpty())
			return false;
	...
	public void handleSpecialBuyEffects(DomCard theCard) {
		...
		for (int i = 0; i < talismanCount; i++) {
			if (new DomCost(4, 0).compareTo(theCard.getCost(getCurrentGame())) >= 0
				...
				DomCard theDouble = getCurrentGame().takeFromSupply(
						theCard.getName());
				if (theDouble != null) {
					gain(theDouble);
				}
		...
		for (int i = 0; i < hoardCount; i++) {
			if (theCard.hasCardType(DomCardType.Victory)) {
				DomCard theGold = getCurrentGame().takeFromSupply(
						DomCardName.Gold);
				if (theGold != null) {
					gain(theGold);
				}
		...
		for (int i = 0; i < game.getEmbargoTokensOn(theCard.name); i++) {
			DomCard theCurse = getCurrentGame().takeFromSupply(
					DomCardName.Curse);
			if (theCurse != null) {
				gain(theCurse);
			}
			...
			for (DomCard theCard : cardsInHand) {
				if (theCard.hasCardType(DomCardType.Treasure)) {
					if (theCardToPlay == null
							|| theCard.getPlayPriority() < theCardToPlay
									.getPlayPriority()) {
	...
	public void doForcedDiscard(int discardsLeft, boolean discardToTopOfDeck) {
		if (!discardToTopOfDeck) {
			...
			while (discardsLeft > 0
					&& !getCardsFromHand(DomCardName.Tunnel).isEmpty()) {
		...
		Collections
				.sort(theTerminalsInHand, DomCard.SORT_FOR_DISCARD_FROM_HAND);
	...
	private void checkForPossibleTrashingBeforeDiscarding(int discardsLeft) {
		...
		if (!getCardsFromHand(DomCardName.Chapel).isEmpty()
			...
			if (theCardsNeededForTrashing.size() > 4)
				...
				theNumberOfCardsNeededForTrashing = theCardsNeededForTrashing
						.size() + 1;
			if (theNumberOfCardsNeededForTrashing > cardsInHand.size()
					- discardsLeft)
				theNumberOfCardsNeededForTrashing = cardsInHand.size()
						- discardsLeft;
		}
		...
		for (int j = 0; j < theNumberOfCardsNeededForTrashing - 1; j++) {
			cardsInHand
					.add(removeCardFromHand(theCardsNeededForTrashing.get(j)));
		}
		cardsInHand
				.add(removeCardFromHand(getCardsFromHand(DomCardType.Trasher)
						.get(0)));
	}
	...
	public boolean isGoingToBuyTopCardInBuyRules(DomCost theTotalMoney) {
		...
		if (wantsToGainOrKeep(theTopCardInBuyRules)
				&& getDesiredCard(theTotalMoney, false) == theTopCardInBuyRules)
			// we don't want to mess with a hand if we're going to buy the top
			// card this turn (although we could)
			return true;
		return false;
	}
	...
	public boolean suicideIfBuys(DomCardName aCardName) {
		...
		if (game.countEmptyPiles() < 2) {
			if (aCardName != DomCardName.Province
					&& aCardName != DomCardName.Colony) {
				return false;
			}
		}
		for (DomPlayer thePlayer : getOpponents()) {
			if (countVictoryPoints() < thePlayer.countVictoryPoints()
					- aCardName.getVictoryValue(this)) {
				return true;
			}
			if (countVictoryPoints() == thePlayer.countVictoryPoints()
					- aCardName.getVictoryValue(this)) {
	...
	/**
	 * @param aPirateShipLevel
	 *            The pirateShipLevel to set.
	 */
	 ...
	 */
	public void passCardToTheLeftForMasquerade(DomCard aCardToPass,
			DomPlayer theMasqueradePlayer) {
		...
		if (theOpponentToTheLeft != theMasqueradePlayer) {
			theOpponentToTheLeft.passCardToTheLeftForMasquerade(
					theOpponentToTheLeft.chooseCardToPass(),
					theMasqueradePlayer);
		}
		if (aCardToPass != null) {
			if (DomEngine.haveToLog)
	...
	public DomPlayer getCopy(String aName) {
		...
		theCopy.prizeBuyRules = new ArrayList<DomBuyRule>(prizeBuyRules);
	...
	public DomPlayer getColonyCopy(String string) {
		...
		for (int i = 0; i < theCopy.buyRules.size(); i++) {
			...
			if (rule.getCardToBuy() == DomCardName.Province) {
				...
				rule.addCondition(new DomBuyCondition(
						DomBotFunction.countCardsInSupply, DomCardName.Colony,
						DomCardType.Action, "0",
						DomBotComparator.smallerOrEqualThan,
						DomBotFunction.constant, DomCardName.Copper,
						DomCardType.Action, "6", DomBotOperator.plus, "0"));
			...
			if (rule.getCardToBuy() == DomCardName.Duchy) {
				...
				rule.addCondition(new DomBuyCondition(
						...
						DomBotComparator.smallerOrEqualThan,
						DomBotFunction.constant, DomCardName.Copper,
						DomCardType.Action, "5", DomBotOperator.plus, "0"));
				theCopy.buyRules.add(i, rule);
			}
			if (rule.getCardToBuy() == DomCardName.Estate) {
				...
				rule.addCondition(new DomBuyCondition(
						...
						DomBotComparator.smallerOrEqualThan,
						DomBotFunction.constant, DomCardName.Copper,
						DomCardType.Action, "2", DomBotOperator.plus, "0"));
				theCopy.buyRules.add(i, rule);
			}
		}
		theCopy.buyRules.add(theLastGreenIndex + 1, new DomBuyRule("Platinum",
				null, null));
	...
	public void addPlayStrategy(String aCard, String aStrategy) {
		if (aStrategy != null) {
			playStrategies.put(DomCardName.valueOf(aCard),
					DomPlayStrategy.valueOf(aStrategy));
		}
	}
	 ...
	 * 
	 * @see
	 * DominionSimulator.DomPlayer#getStrategyFor(DominionSimulator.DomCard)
	 */
	 ...
	 * @return
	 */
	public List<DomBuyRule> getBuyRules() {
		if (possessor != null)
	...
	public DomCardName getDesiredCard(DomCost anAvailableCurrency,
			boolean costExact) {
	...
	public DomCardName getDesiredCard(DomCardType aDesiredType,
			DomCost anAvailableCurrency, boolean costExact,
			boolean noConstraints, DomCardType aForbiddenType) {
		for (DomBuyRule theRule : getBuyRules()) {
			if (aDesiredType != null
					&& !theRule.getCardToBuy().hasCardType(aDesiredType))
				continue;
			if (aForbiddenType != null
					&& theRule.getCardToBuy().hasCardType(aForbiddenType))
				continue;
			boolean wantsToGain = true;
			for (DomBuyCondition theCondition : theRule.getBuyConditions()) {
				// if (noConstraints &&
				// theCondition.getLeftFunction()==DomBotFunction.isActionPhase){
				// //TODO probably remove this...
				// //be very careful: this fix was added so Estates can be
				// gained with Remodel, but their trash priority stays low
				// //so eg Coppers aren't remodeled into Estates before Estates
				// into Caravans
				// wantsToGain=false;
				// break;
				// }
			...
			if (wantsToGain) {
				if ((!costExact && anAvailableCurrency.compareTo(theRule
						.getCardToBuy().getCost(getCurrentGame())) >= 0)
						|| (costExact && anAvailableCurrency.compareTo(theRule
								.getCardToBuy().getCost(getCurrentGame())) == 0)) {
					if (noConstraints
							|| (!suicideIfBuys(theRule.getCardToBuy()) && getCurrentGame()
									.countInSupply(theRule.getCardToBuy()) > 0))
	...
	public void addBuyRuleFor(DomCardName aCard) {
		for (DomBuyRule theRule : getBuyRules()) {
			if (theRule.getCardToBuy().hasCardType(DomCardType.Victory))
				continue;
			if (aCard.getCost().getCoins() >= theRule.getCardToBuy().getCost()
					.getCoins()
	...
	private void insertBuyRule(DomCardName aCard, DomBuyRule theRule) {
		...
		if (aCard == DomCardName.Young_Witch)
			theBane = ((DomCardName) DomCardName.getPossibleBaneCards()[(int) Math
					.random() * DomCardName.getPossibleBaneCards().length])
					.name();
		DomBuyRule theNewRule = new DomBuyRule(aCard.name(),
				DomPlayStrategy.standard.name(), theBane);
		...
		if (aCard.getPotionCost() > 0) {
			theNewRule = new DomBuyRule(DomCardName.Potion.name(),
					DomPlayStrategy.standard.name(), null);
			theCondition = new DomBuyCondition();
			theCondition.addLeftHand("countCardsInDeck",
					DomCardName.Potion.name());
			theCondition.addComparator("smallerThan");
			theCondition.addRightHand("constant", "1");
			theNewRule.addCondition(theCondition);
			getBuyRules().add(getBuyRules().indexOf(theRule), theNewRule);
	...
	public boolean removingReducesBuyingPower(DomCard aCardToTrash) {
		...
		if (theValue.compareTo(DomCost.ZERO) > 0) {
			...
			if (getDesiredCard(theTotalCurrency, false) != getDesiredCard(
					theReducedCurrency, false))
				return true;
		}
		return false;
	}
	...
	public String getXML() {
		...
		theXML.append("<player name=\"").append(name).append("\"")
				.append(newline);
		theXML.append(" author=\"").append(author).append("\"").append(newline);
		theXML.append(" description=\"")
				.append(description.replace(newline, "XXXX")).append("\"");
		theXML.append(">").append(newline);
		for (DomBotType botType : getTypes()) {
			theXML.append(" <type name=\"").append(botType.name())
					.append("\"/>").append(newline);
		}
		if (myStartState != null)
			theXML.append(myStartState.getXML());
		if (!mySuggestedBoardCards.isEmpty()) {
			theXML.append("  <board contents=\"")
					.append(mySuggestedBoardCards.toString().replaceAll(
							"\\[|\\]", "")).append("\"");
	...
	public List<DomBuyRule> getPrizeBuyRules() {
	...
	public boolean stillInEarlyGame() {
		for (DomCardName cardName : DomCardName.values()) {
			if (cardName == DomCardName.Estate)
				continue;
			if (cardName.hasCardType(DomCardType.Victory)
					&& cardName.getDiscardPriority(1) < 10)
