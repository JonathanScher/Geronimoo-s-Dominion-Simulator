...
public class DomBoard extends EnumMap<DomCardName, PileOfCards> implements
		PileOfCardsObserver {
	...
	private EnumMap<DomCardName, Integer> embargoTokens = new EnumMap<DomCardName, Integer>(
			DomCardName.class);
	...
	@Override
	public PileOfCards put(DomCardName key, PileOfCards value) {
			if (value.size() == 0) {
				emptyPiles++;
			}
	...
	public void addCardPile(DomCardName aCardName) {
		if (get(aCardName) != null)
			return;
		put(aCardName, new PileOfCards(this));
		int theNumber = 10;
	...
	public DomCard take(DomCardName aCardName) {
		...
		PileOfCards theList = get(aCardName);
		if (theList == null)
			return null;
		if (aCardName.hasCardType(DomCardType.Victory)) {
		...
		if (aCard.isFromBlackMarket()) {
			...
			if (aCard.hasCardType(DomCardType.Prize)) {
				...
				if (get(aCard.getName()) == null) {
					put(aCard.getName(), new PileOfCards(this));
				}
	...
	public DomCardName getRandomCardWithEmbargoToken() {
		...
		ArrayList<DomCardName> theCards = new ArrayList<DomCardName>(
				Arrays.asList(embargoTokens.keySet()
						.toArray(new DomCardName[0])));
	...
	public String getEmbargoInfo() {
		...
		for (DomCardName theCard : embargoTokens.keySet()) {
			theInfo += theCard.toHTML() + " ("
					+ embargoTokens.get(theCard).toString() + "), ";
		}
		return theInfo;
	}
	...
	public DomCardName getBestCardInSupplyFor(DomPlayer aPlayer,
		...
		for (DomCardName theCardName : keySet()) {
			if (get(theCardName) != null
					...
					&& (aForbiddenType == null || !theCardName
							.hasCardType(aForbiddenType))
					&& ((!anExactCost && domCost.compareTo(theCardName
							.getCost(aPlayer.getCurrentGame())) >= 0) || (anExactCost && domCost
							.compareTo(theCardName.getCost(aPlayer
									.getCurrentGame())) == 0))
					&& (theCardToGet == null || theCardName
							.getTrashPriority(aPlayer) > theCardToGet
							.getTrashPriority(aPlayer))) {
	...
	public DomCardName getCardForSwindler(DomPlayer aPlayer, DomCost domCost) {
		...
		for (DomCardName theCardName : keySet()) {
			if (get(theCardName) != null
					&& !get(theCardName).isEmpty()
					&& theCardName.getCost(aPlayer.getCurrentGame()).compareTo(
							domCost) == 0
					&& (theCardToGet == null || theCardName
							.getTrashPriority(aPlayer) < theCardToGet
							.getTrashPriority(aPlayer))) {
	...
	public int getGainsNeededToEndGame() {
		...
		int theCountGainsToEndGame = theCounts.get(0) + theCounts.get(1)
				+ theCounts.get(2);
		if (get(DomCardName.Colony) != null
				&& count(DomCardName.Colony) < theCountGainsToEndGame)
