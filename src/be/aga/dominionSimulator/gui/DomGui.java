...
public class DomGui extends JFrame implements ActionListener {
	...
	public DomGui(DomEngine anEngine, LogHandler logHandler) {
	...
	private void initializeComponent() {
		...
		myTopSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true,
				new JScrollPane(getControlPanel()),
				myVPLineChart.getChartPanel());
		myBottomSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true,
				myBarChart.getChartPanel(), myMoneyLineChart.getChartPanel());
		myBigSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true,
				myTopSplit, myBottomSplit);
	...
	private JPanel getControlPanel() {
		...
		for (int i = 1; i < 5; i++) {
			...
			for (Enumeration<AbstractButton> theEnum = theBTNGroup
					.getElements(); theEnum.hasMoreElements();) {
				theCons.gridx++;
		...
		myOrderBox
				.setToolTipText("If unchecked, the players will be moved to a new random position for each game");
		...
		JButton theBotSelector = new JButton() {
			...
			public String getToolTipText() {
				if (getSelectedPlayer(this) != null) {
					String theDescription = getSelectedPlayer(this)
							.getDescription().replaceAll("XXXX", "<br>");
	...
	private JButton getEditCreateButton(JButton theBotSelector) {
		...
		if (theBTN == null) {
			...
			theBTN.setMnemonic(theBTN.getText().charAt(
					myBotSelectors.indexOf(theBotSelector)));
	...
	public void actionPerformed(ActionEvent aE) {
		if (aE.getActionCommand().equals("Select")) {
			...
			new DomBotSelector(myEngine,
					getSelectedPlayer((JButton) aE.getSource()));
		}
		...
		if (aE.getActionCommand().equals("About")) {
			JOptionPane.showMessageDialog(this, getAboutPanel(), "About",
					JOptionPane.PLAIN_MESSAGE);
		}
		if (aE.getActionCommand().equals("WebHelp")) {
			try {
				Desktop.getDesktop().browse(
						new URI("http://dominionsimulator.wordpress.com"));
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "Can not open web site!",
		...
		if (aE.getActionCommand().equals("CopyPaste")) {
			...
			if (theStrategy == null) {
				DomPlayer theNewPlayer = myEngine
						.loadUserBotsFromXML(new InputSource(new StringReader(
								getClipboard())));
	...
	private DomPlayer getSelectedPlayer(JButton theSelector) {
		...
		if (theName == null || theName.equals(""))
			return null;
		for (Object player : myEngine
				.getBots(new DomBotType[] { DomBotType.Bot })) {
		...
		try {
			if (t != null && t.isDataFlavorSupported(DataFlavor.stringFlavor)) {
				...
				JOptionPane.showMessageDialog(this,
						"Your clipboard does not contain a valid XML!", "",
						JOptionPane.ERROR_MESSAGE);
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(this,
	...
	private void showSampleGame() {
		...
		try {
			...
			out.write("<FONT size=2 face=\"Arial\">" + logHandler.getMyLog());
			out.close();
			Desktop.getDesktop().open(tempfile);
			tempfile.deleteOnExit();
		} catch (IOException e) {
			JOptionPane.showMessageDialog(this, getSampleGamePanel(),
					"Sample Game", JOptionPane.PLAIN_MESSAGE);
		}
	}
	...
	private DomPlayer colonize(DomPlayer theSelectedBot) {
		...
		DomPlayer theNewPlayer = theSelectedBot.getColonyCopy(theSelectedBot
				.toString() + "(Col)");
	...
	private void startSimulation(ActionEvent aE) {
		...
		for (JButton theSelector : myBotSelectors) {
			...
			DomPlayer thePlayer = theSelectedBot
					.getCopy(theSelectedBot + "(Plr "
							+ (myBotSelectors.indexOf(theSelector) + 1) + ")");
			int j = 0;
			for (Enumeration<AbstractButton> theEnum = myStartStateButtonGroups
					.get(theSelector).getElements(); theEnum.hasMoreElements(); j++) {
		...
		if (!myPlayers.isEmpty()) {
			...
			myEngine.startSimulation(thePlayers, myOrderBox.isSelected(),
					theNumber, showLog);
	...
	public void setVPLineChart(DomLineChart aChart) {
		myVPLineChart.getChartPanel().setChart(
				aChart.getChartPanel().getChart());
	}
	...
	public void setMoneyLineChart(DomLineChart aChart) {
		myMoneyLineChart.getChartPanel().setChart(
				aChart.getChartPanel().getChart());
	}
	...
	public JPanel getSampleGamePanel() {
		...
		a.setText("<FONT size=3 face=\"Arial\">" + logHandler.getMyLog());
	...
	private void updateSelectionButtonText(JButton aSelector, String aText) {
		aSelector.setFont(aSelector.getFont().deriveFont(
	...
	private JMenuBar createMenu() {
		...
		JMenuItem webHelp = new JMenuItem(
				"http://dominionsimulator.wordpress.com");
	...
	public static void addHeavyLabel(final JPanel thePanel,
			final GridBagConstraints theCons) {
		theCons.weightx = 100;
		theCons.weighty = 100;
